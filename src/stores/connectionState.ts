...
export const useConnectionStateStore = useInitializableStore(
	defineStore("connectionState", {
		...
		getters: {
			...
			reconnectPossible(state) {
				...
				return (
					!state.inProgress &&
					(settings.data.debug ||
						version.branch === "internal" ||
						(!state.wasConnected && !state.connected && state.disconnected))
				);
			},
		},
		actions: {
			...
			init() {
				...
				alt.on(
					"connection:setCacheKeys",
					(resourceCacheKey?: string, dataCacheKey?: string) => {
						this.connectedCacheKeys = [resourceCacheKey, dataCacheKey].filter(
							Boolean,
						) as string[];
					},
				);
				...
				alt.on(
					"connection:downloadingResources",
					(bytesDownloaded: number, bytesTotal: number, speed?: number) => {
						this.reset();
				...
				alt.on(
					"connection:validatingResources",
					(resourcesValidated: number, resourcesTotal: number) => {
						this.reset();
				...
				alt.on(
					"connection:downloadingRuntimes",
					(bytesDownloaded: number, bytesTotal: number, speed?: number) => {
						this.reset();
				...
				alt.on(
					"connection:downloadingAdditionalResources",
					(bytesDownloaded: number, bytesTotal: number, speed?: number) => {
						this.reset();
						this.connected = true;
				...
				alt.on(
					"connection:validatingAdditionalResources",
					(bytesDownloaded: number, bytesTotal: number, speed?: number) => {
						this.reset();
						this.connected = true;
						this.action = "SERVER_UPDATE";
