...
export const useLocalization = defineStore("localization", {
	...
	actions: {
		init(locales: Locale[], defaultLocale = "en") {
			...
			watch(
				() => !!this.currentLocale.rtl && !settings.data.disableRtl,
				(value) => (this.dir = value ? "rtl" : "ltr"),
			);
		...
		prepareLocale(locale: Locale) {
			...
			const defaultLocale = this.locales.get(this.defaultLocale) ?? {
	...
	getters: {
		t(this: any, state) {
			return (key: string, ...args: (string | number)[]) =>
				this.formatString(
					state.currentLocale.preparedStrings?.get(key) || key,
					args,
				);
		},
		tRaw(this: any, state) {
			return (key: string) =>
				String(state.currentLocale.preparedStrings?.get(key) || key);
		},
		...
		tArgsFromStr(this: any, state) {
			return (key: string) => {
				...
				try {
					...
					if (!state.currentLocale.preparedStrings?.has(localeKey))
						return fallback;
					const args = JSON.parse(elements[1] ?? "");
					if (!args) return fallback;
					return this.formatString(
						state.currentLocale.preparedStrings?.get(localeKey),
						args,
					);
				} catch {
					return fallback;
		...
		tPlural(this: any, state) {
			...
			return (
				...
				return this.formatString(
					strings?.get(`${key}:${rules.select(numValue).toUpperCase()}`) ||
						strings?.get(key) ||
