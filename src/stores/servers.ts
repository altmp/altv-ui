...
export type OnlineServerWithoutGroup = Omit<OnlineServer, "group">;

export type ServerGroup = {
	id: string;
	name: string;
	servers: OnlineServerWithoutGroup[];
	totalPlayersCount: number;
	totalMaxPlayersCount: number;
	verified: boolean;
	promoted: boolean;
};
...
export interface ServersStore {
	...
	groupedServers: (ServerGroup | OnlineServer)[];
	serversLookup: Record<string, number>;
	privateServers: Record<string, OnlineServer>;
...
export const isGroup = (
	serverOrGroup: OnlineServer | ServerGroup
): serverOrGroup is ServerGroup => {
	return "servers" in serverOrGroup;
};

const groupServers = (servers: OnlineServer[]) => {
	const serversWithoutGroups: OnlineServer[] = [];
	const groups: {
		id: string;
		name: string;
		servers: OnlineServerWithoutGroup[];
	...
	for (let i = 0; i < servers.length; i++) {
		...
		const serverWithoutGroup = { ...server, group: undefined };
		...
		if (groupIndex === -1) {
			groups.push({
				...
				servers: [serverWithoutGroup],
			});
		} else {
			groups[groupIndex]!.servers.push(serverWithoutGroup);
		}
	}
	...
	const result = [
		...groups.map((group) => ({
			...group,
			totalPlayersCount: group.servers.reduce(
				(acc, server) => acc + server.playersCount,
				0
			),
			totalMaxPlayersCount: group.servers.reduce(
...
export const useServersStore = useInitializableStore(
	defineStore("servers", {
		...
		getters: {
			...
			getServer:
				(state) =>
				(id: string): OnlineServer | undefined =>
		...
		actions: {
			...
			async reload() {
				...
				try {
					...
					this.servers = (await res.json()) as OnlineServer[];
