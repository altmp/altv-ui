...
function sanitize(content: string) {
    return sanitizeHtml(content, {
        ...
        allowedAttributes: {
            ...
            span: ['data-timestamp', 'data-mention', 'data-localized', 'data-spoiler', 'data-images']
        }
    });
}

function formatDate(date: string) {
    return moment(date).locale(locale.currentLocale.intlCode ?? 'en').format('LT · LL');
}
...
const hook = () => {
    ...
    for (let el of Array.from(value.querySelectorAll('span[data-timestamp]'))) {
        const type = el.getAttribute('data-timestamp') ?? 'f';
        const time = moment.unix(+el.innerHTML).locale(locale.currentLocale.intlCode ?? 'en');

        el.innerHTML = type == "R" ? time.fromNow()! : time.format(formats[type] ?? formats.f);
    ...
    for (let el of Array.from(value.querySelectorAll('span[data-images]'))) {
        el.addEventListener('wheel', (e) => {
            ...
            let index = Array.from(el.children).findIndex(e => e.getBoundingClientRect().left + 6 >= rect.left);
            index += mod;
            if (index < 0) index = 0;
            if (index >= el.children.length) index = el.children.length - 1;
            if ((el.scrollLeft == 0 && mod == -1) || (el.scrollLeft + el.clientWidth >= el.scrollWidth && mod == 1)) return;
            event.preventDefault();
            el.children[index].scrollIntoView({block: "nearest", inline: "start", behavior: "smooth"});
        });
    }
...
<template>
    <div class="news" v-if="version.rss" dir="ltr" ref="mainRef">
        <block-container class="news__frame" :class="{ 'news__frame--no-link': !el.link }" v-for="el in items" @click="el.link && openLink(el.link)">
...
.news {
    * {
    ...
    &__frame {
        ...
        gap: u(24);
        overflow: hidden;
        transition: opacity 0.2s ease-in-out, outline-color 0.2s ease-in-out, transform 0.2s ease;
        cursor: pointer;
        padding: u(24);
    ...
    &__content {
        ...
        :deep {
            span[data-timestamp], span[data-mention], span[data-spoiler] {
                display: none;
            }
