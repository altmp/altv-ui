...
<template>
	<div class="menu__group">
		...
		<alt-input
			...
			v-model="settings.data.name"
			@change="settings.save('name')"
		></alt-input>
	</div>
	...
	<div class="menu__group">
		...
		<alt-checkbox
			...
			v-if="locale.currentLocale.rtl"
			@change="settings.save('disableRtl')"
			:label="t('DISABLE_RTL')"
		></alt-checkbox>
	</div>
	...
	<div class="menu__group">
		...
		<alt-dropdown
			:disabled="!connection.newConnectionPossible"
			v-model="settings.data.region"
			:elements="regions"
			@change="settings.save('region')"
		></alt-dropdown>
	</div>

	<alt-slider
		v-model="settings.data.uiVolume"
		@change="settings.save('uiVolume')"
		:label="t('UI_VOLUME')"
	></alt-slider>
	...
	<div class="menu__group">
		...
		<alt-input
			v-model="speedLimit"
			:disabled="connection.inProgress"
			:placeholder="t('UNLIMITED')"
			@keydown.enter="saveSpeedLimit"
			@blur="saveSpeedLimit"
			:pattern="/\D/g"
		></alt-input>
	</div>
	...
	<div class="menu__group">
		...
		<alt-checkbox
			v-model="settings.data.discordRichPresence"
			@change="settings.save('discordRichPresence')"
			:label="t('DISCORD_RICH_PRESENCE')"
		></alt-checkbox>
		<alt-checkbox
			v-model="settings.data.netgraphEnabled"
			@change="settings.save('netgraphEnabled')"
			:label="t('NETGRAPH')"
		></alt-checkbox>
		<alt-checkbox
			v-model="settings.data.useExternalConsole"
			@change="settings.save('useExternalConsole')"
			:label="t('USE_EXTERNAL_CONSOLE')"
		></alt-checkbox>
		<alt-checkbox
			v-model="settings.data.audioFrameLimiter"
			@change="settings.save('audioFrameLimiter')"
			:label="t('AUDIO_FRAME_LIMITER')"
		/>
	</div>

	<alt-button
		v-if="settings.data.launcherSkin !== ''"
		color="gray"
		@click="resetSkin"
		>{{ t("RESTORE_DEFAULT_SKIN") }}</alt-button
	>
</template>
