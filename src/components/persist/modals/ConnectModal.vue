...
function connect() {
    ...
    alt.emit('connection:connect', server.host, server.port, server.cdnUrl, server.earlyAuthUrl, password.value, server.id, server.name, hasSkin.value && skinChecked.value);
    modal.close();
}
</script>

<template>
    <div class="connect" :class="{ 'connect--wide': bannerError || modalProps.server.bannerUrl }">
        <div v-if="bannerError" class="connect__banner connect__banner--error"></div>
        <template v-else-if="modalProps.server.bannerUrl">
            <img class="connect__banner" :class="{ 'connect__banner--loading': bannerLoading }" :src="modalProps.server.bannerUrl" @load="bannerLoading = false" @error="bannerError = true" />
            <div class="connect__banner connect__banner--placeholder" v-if="bannerLoading"></div>
        </template>
            ...
            <div class="connect__data">
                <div class="info">
                    ...
                    <div class="info__element" v-if="modalProps.server.website">
                        <a :href="websiteUrl" target="_blank" @click="playMoveSound" @mouseenter="playHoverSound">{{ modalProps.server.website }}</a>
                    </div>
                </div>
...
.connect {
    ...
    &__password, &__password :deep(input) {
        width: u(240);
    }
