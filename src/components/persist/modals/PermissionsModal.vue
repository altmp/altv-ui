...
<template>
    <div class="modal">
        ...
        <div class="modal__permissionGroups">
            <div class="permissions permissions--required" v-if="modalProps.required && modalProps.required.length">
                <div class="permissions__title">{{ t('REQUIRED_PERMISSIONS') }}</div>
                <div class="permissions__list">
                    <alt-checkbox :label="getPermissionName(perm)" disabled :model-value="true" v-for="perm in modalProps.required" />
                </div>
            </div>
            <div class="permissions" v-if="modalProps.optional && modalProps.optional.length">
                <div class="permissions__title">{{ t('OPTIONAL_PERMISSIONS') }}</div>
                <div class="permissions__list">
                    <alt-checkbox :label="getPermissionName(perm)" v-model="values[perm]" v-for="perm in modalProps.optional" />
                </div>
            </div>
        </div>
        <div class="modal__actions">
            ...
            <a href="#" v-if="modalProps.optional.length" @click="acceptRequired();" @mouseenter="playHoverSound">{{ t('ALLOWED_REQUIRED_ONLY') }}</a>
        </div>
        <div class="modal__additionalAction">
        </div>
    </div>
</template>
