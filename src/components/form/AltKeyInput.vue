...
watch(() => active.value, (newActive, oldActive, onCleanup) => {
    if (!newActive) return;
...
function getKeyName(value: number) {
    return (keys as Record<number, string>)[value] || String.fromCharCode(value) || "UNKNOWN";
}
</script>

<template>
    <div v-bind="$attrs" @click.stop="() => {active = !active; playClickSound()}" :class="{ active }" @mouseenter="playHoverSound">{{ active ? "..." : getKeyName(modelValue) }}</div>
</template>
...
div {
    ...
    transition: background-color 0.1s ease-in-out, transform 0.2s ease;
