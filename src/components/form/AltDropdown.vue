    ...
    <div class="dropdown" :class="{ open: open.state }" @click.stop="toggle(); playClickSound()" @mouseenter="playHoverSound">
        ...
        <div v-if="open.state" class="menu">
            <div class="element"
                 ...
                 v-for="el in elements.filter(e => e.label.toLowerCase().startsWith(search.toLowerCase())
                        || e.value.toLowerCase().startsWith(search.toLowerCase()))"
                 @click.stop="() => { $emit('update:modelValue', el.value); $emit('change', el.value); open.state = false; playClickSound(); }" @mouseenter="playHoverSound">
                {{ el.label }}
            </div>
        </div>
    </div>
